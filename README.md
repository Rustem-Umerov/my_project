...
Приложение для работы с банковскими транзакциями. Позволяет:
...
- Фильтровать транзакции по валюте (например, показывать только рублевые).
- Фильтровать транзакции по ключевым словам в описании.

## Функциональность 

1. **Загрузка данных**  
   Пользователь выбирает источник данных, после чего приложение считывает информацию из файла.
2. **Фильтрация по статусу**  
   После ввода статуса (например, EXECUTED) происходит фильтрация транзакций.
3. **Сортировка по дате**  
   Опциональная сортировка по дате позволяет упорядочить операции в удобном для анализа виде.
4. **Дополнительные фильтры**  
   Пользователь может выбрать вывод транзакций по валюте или отфильтровать операции по ключевым словам.
...
## Модуль banking_operations.py

### Данный модуль содержит:
- #### Функции process_bank_search и process_bank_operations

Функция process_bank_search, принимает на вход, список словарей с данными о банковских операциях и строку для поиска.
...
```
# Пример данных для демонстрации работы функции:
transactions = [
    {"description": "Перевод с карты на карту", "amount": 100},
    {"description": "Открытие вклада", "amount": 200},
    {"description": "Перевод с карты", "amount": 150},
]

search = "вклад"
filtered_transactions = process_bank_search(transactions, search)
print("Результат поиска:", filtered_transactions)
# Ожидаемый вывод:
# Результат поиска: [{'description': 'Открытие вклада'
```
...
transactions = [
    {"description": "перевод с карты", "amount": 150},
    {"description": "открытие вклада", "amount": 250},
    {"description": "перевод с карты", "amount": 100},
    {"description": "Перевод с карты", "amount": 300},
]

categories = ["перевод с карты", "открытие вклада"]
operations_summary = process_bank_operations(transactions, categories)
print("Результат группировки:", operations_summary)
# Ожидаемый вывод:
# Результат группировки: {'перевод с карты': 3, 'открытие вклада': 1}
```
